...
void setup_gpu(
    float **obj, float **gra, float **dir, float **mes, float **res, float **ray) {
  int numproc;
  int myid;
  ...
  if (myid == 0) {
    ...
    printf(
        "Maximum block dimensions: %dx%dx%d\n", deviceProp.maxThreadsDim[0],
        deviceProp.maxThreadsDim[1], deviceProp.maxThreadsDim[2]);
    printf(
        "Maximum grid dimensions: %dx%dx%d\n", deviceProp.maxGridSize[0],
  ...
  cudaMemcpy(
      proj_blockdispl_d, proj_blockdispl, sizeof(int) * (proj_numblocks + 1),
      cudaMemcpyHostToDevice);
  cudaMemcpy(
      proj_buffdispl_d, proj_buffdispl, sizeof(int) * (proj_blocknztot + 1),
      cudaMemcpyHostToDevice);
  cudaMemcpy(
      proj_buffmap_d, proj_buffmap, sizeof(int) * proj_blocknztot * proj_buffsize,
      cudaMemcpyHostToDevice);
  cudaMemcpy(
      proj_buffindex_d, proj_buffindex, sizeof(short) * proj_buffnztot * proj_blocksize,
      cudaMemcpyHostToDevice);
  cudaMemcpy(
      proj_buffvalue_d, proj_buffvalue, sizeof(float) * proj_buffnztot * proj_blocksize,
      cudaMemcpyHostToDevice);

  /*for(int block = 0; block < proj_numblocks; block++)
    for(int buff = proj_blockdispl[block]; buff < proj_blockdispl[block+1]; buff++){
      printf("                                              block %d buff
  %d\n",block,buff); for(int m = proj_buffdispl[buff]; m < proj_buffdispl[buff+1]; m++){
  ...
  printf("buffnztot: %d blocksize: %d
  %d\n",proj_buffnztot,proj_blocksize,proj_buffnztot*proj_blocksize);
  printf("blocknztot: %d buffsize: %d
  ...
  cudaMemcpy(
      back_blockdispl_d, back_blockdispl, sizeof(int) * (back_numblocks + 1),
      cudaMemcpyHostToDevice);
  cudaMemcpy(
      back_buffdispl_d, back_buffdispl, sizeof(int) * (back_blocknztot + 1),
      cudaMemcpyHostToDevice);
  cudaMemcpy(
      back_buffmap_d, back_buffmap, sizeof(int) * back_blocknztot * back_buffsize,
      cudaMemcpyHostToDevice);
  cudaMemcpy(
      back_buffindex_d, back_buffindex, sizeof(short) * back_buffnztot * back_blocksize,
      cudaMemcpyHostToDevice);
  cudaMemcpy(
      back_buffvalue_d, back_buffvalue, sizeof(float) * back_buffnztot * back_blocksize,
      cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(
      rayraystart_d, rayraystart, sizeof(int) * (mynumray + 1), cudaMemcpyHostToDevice);
...
void projection(float *mes, float *obj) {
  ...
  double timef = MPI_Wtime();
  {
    ...
    kernel_SpMV_buffered<<<numblocks, blocksize, sizeof(float) * buffsize>>>(
        raypart_d, tomogram_d, buffindex, buffvalue, raynumout, blockdispl, buffdispl,
        buffmap, buffsize);
  ...
  }
  {
    ...
    MPI_Alltoallv(
        raypart, raysendcount, raysendstart, MPI_FLOAT, raybuff, rayrecvcount,
        rayrecvstart, MPI_FLOAT, MPI_COMM_WORLD);
  ...
  {
    double time = MPI_Wtime();
    kernel_SpReduce<<<(mynumray + 255) / 256, 256>>>(
        sinogram_d, raybuff_d, rayraystart_d, rayrayind_d, mynumray);
...
void backprojection(float *gra, float *res) {
  ...
  double timeb = MPI_Wtime();
  {
    ...
    kernel_SpGather<<<(raynuminc + 255) / 256, 256>>>(
        raybuff_d, sinogram_d, rayindray_d, raynuminc);
    cudaDeviceSynchronize();
    MPI_Barrier(MPI_COMM_WORLD);
  ...
  }
  {
    ...
    MPI_Alltoallv(
        raybuff, rayrecvcount, rayrecvstart, MPI_FLOAT, raypart, raysendcount,
        raysendstart, MPI_FLOAT, MPI_COMM_WORLD);
  ...
  }
  {
    ...
    cudaEventRecord(start);
    kernel_SpMV_buffered<<<numblocks, blocksize, sizeof(float) * buffsize>>>(
        tomogram_d, raypart_d, buffindex, buffvalue, mynumpix, blockdispl, buffdispl,
        buffmap, buffsize);
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    float milliseconds = 0;
...
__global__ void kernel_SpMV_buffered(
    float *y,
    float *x,
    short *index,
    float *value,
