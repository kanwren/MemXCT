...
#pragma omp parallel
  ...
  if (myid == 0) {
    ...
    printf("\n");
    printf(
        "INTEGER: %d, FLOAT: %d, LONG: %d, SHORT: %d, POINTER: %d\n", (int)sizeof(int),
        (int)sizeof(float), (int)sizeof(long), (int)sizeof(short),
        (int)sizeof(complex<float> *));
    printf(
        "APPROX. MEMORY     TOTAL: %e GB\n",
        (numthe * numrho) / 1024.0 / 1024.0 / 1024.0 * numrho * 16.0 * 1.5);
    printf(
        "APPROX. MEMORY PER PROC.: %e GB\n",
...
#pragma omp parallel for
  ...
  int maxnumray = 0;
  for (int p = 0; p < numproc; p++) {
    if (numpixs[p] > maxnumpix) {
      maxnumpix = numpixs[p];
    }
    if (numrays[p] > maxnumray) {
      maxnumray = numrays[p];
    }
  }
  if (myid == 0) {
    for (int p = 0; p < numproc; p++) {
      printf(
          "proc: %d numspats: %d numpixs: %d numspecs: %d numrays: %d\n", p, numspats[p],
          numpixs[p], numspecs[p], numrays[p]);
    }
...
#pragma omp parallel for
    for (int k = 0; k < mynumray; k++) {
      ...
      for (int tile = spatstart[p]; tile < spatstart[p] + numspats[p]; tile++) {
        ...
        if (domain[1] > xstart + numx * pixsize) {
          domain[1] = xstart + numx * pixsize;
        }
        if (domain[3] > ystart + numy * pixsize) {
          domain[3] = ystart + numy * pixsize;
        }
  ...
  MPI_Alltoall(
      rayrecvcount, 1, MPI_INTEGER, raysendcount, 1, MPI_INTEGER, MPI_COMM_WORLD);
  rayrecvstart[0] = 0;
  raysendstart[0] = 0;
  for (int p = 1; p < numproc; p++) {
...
#pragma omp parallel for
  ...
  delete[] rayrecvtemp;
  MPI_Alltoallv(
      rayrecvlist, rayrecvcount, rayrecvstart, MPI_INTEGER, raysendlist, raysendcount,
      raysendstart, MPI_INTEGER, MPI_COMM_WORLD);
...
#pragma omp parallel for
  ...
  MPI_Alltoallv(
      raycoorinc, rayrecvcount, rayrecvstart, MPI_COMPLEX, raycoorout, raysendcount,
      raysendstart, MPI_COMPLEX, MPI_COMM_WORLD);
  delete[] raycoor;
  MPI_Barrier(MPI_COMM_WORLD);
...
#pragma omp parallel for schedule(dynamic, proj_blocksize)
    ...
    if (myid == 0) {
      printf(
          "CSR STORAGE: %lu (%f GB) rownzmax %d\n", rownzall,
          rownzall * sizeof(float) * 2 / 1024.0 / 1024.0 / 1024.0, rownzmax);
    }
...
#pragma omp parallel for schedule(dynamic)
    for (int block = 0; block < proj_numblocks; block++) {
      for (int k = block * proj_blocksize;
           k < (block + 1) * proj_blocksize && k < raynumout; k++) {
        float rho   = raycoorout[k].real();
        float theta = raycoorout[k].imag();
...
#pragma omp parallel for schedule(dynamic)
    for (int block = 0; block < back_numblocks; block++) {
      for (int n = block * back_blocksize;
           n < (block + 1) * back_blocksize && n < mynumpix; n++) {
        float domain[4];
...
#pragma omp parallel for
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  {
    ...
    if (myid == 0) {
      printf(
          "recon: %e proj: %e (%e %e %e) backproj: %e (%e %e %e) ctime: %e wtime: %e\n",
          rtime, ftime, fktime, aftime, frtime, btime, brtime, abtime, bktime, ctime,
          wtime);
    }
    ...
    if (myid == 0) {
      printf(
          "proj: %e s (%f GFLOPS) backproj: %e s (%f GFLOPS) avGFLOPS: %f totGFLOPS: "
          "%f\n",
          fktime, projflops, bktime, backflops, totflops, totflops * numproc);
    }
    if (myid == 0) {
      printf(
          "proj: %f GB/s back: %f GB/s av: %f GB/s tot: %f GB/s\n", projbw, backbw, totbw,
          totbw * numproc);
    }
  }
